...
Контест для решения и проверок задач по программированию
...
Конкретная задача для решения в контесте для полученя рейтинговый баллов.
...
Посылка отправленная тестирующей системе для проверки решения конкретной задачи пользователем.
...
Отчет - ответ тестирующей системы на отправленную посылку пользователем

Сущность Отчет содержит следующие атрибуты:
* Id
* Код ответа
* Посылка
  
Посылка являются связью, один ко одному, сущности Отчет и Посылка.
